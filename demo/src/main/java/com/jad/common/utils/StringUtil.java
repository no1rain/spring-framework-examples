...
public class StringUtil {
    ...
    */
    public static String lpad(String s, int n, String replace){
        ...
        if (!isEmpty(s)){
            int iByteSize = getByte(s);
            if (iByteSize > n){
                return s.substring(0, n);
            }else if (iByteSize == n){
                return s;
            }else{
                int iPadLength = n - iByteSize;
                sbBuffer = new StringBuffer();
                for (int j = 0; j < iPadLength; j++){
                    sbBuffer.append(replace);
                }
                sbBuffer.append(s);
    ...
    /**
     * 주어진 길이(n)만큼 주어진 문자(replace)를 s의 오른쪽에 붙혀서 보내준다.
     * ex) rpad("abc", 5, '^') ==> "abc^^"
     *     rpad("abcdefghi", 5, '^') ==> "abcde"
     *     rpad(null, 5, '^') ==> "^^^^^"
     *
     * @param s
     * @param n
